...
def speichern():
    ...
    np.savetxt((dateiname), [p for p in zip(stammdaten, tDatum, tAnzahl, tKurs, tIstKauf, vJahr, vHoehe)], delimiter=',', fmt='%s')
...
def laden():
    ...
    for reihe in daten:
        if reihe[1] != '':
            transaktionen.append(Transaktion(int(reihe[2]),float(reihe[3]),reihe[1],str2bool(reihe[4])))
        if len(reihe) > 6:
            if reihe[5]!='':
                vorabpauschalen.append(Vorabpauschale(float(reihe[6]), int(reihe[5])))
    buttonsAktivieren()
    update()
...
def berechneAktuellePositionen():
    ...
    for transaktion in transaktionen:
        if transaktion.istKauf:
            aktuellePositionen.append(Position(transaktion.anzahl, transaktion.kurs, transaktion.datum))
        else:
...
def update():
    ...
    for transaktion in transaktionen:
        if transaktion.istKauf:
            transaktionenListBox.insert("end", "Kauf "+str(transaktion.datum)+": "+str(transaktion.anzahl)+" Anteile @ "+str(transaktion.kurs)+"€")
        else:
            transaktionenListBox.insert("end", "Verkauf "+str(transaktion.datum)+": "+str(transaktion.anzahl)+" Anteile @ "+str(transaktion.kurs)+"€")
...
class Transaktion:
    ...
    datumWert = 0
    istKauf = True
    def __init__(self, anzahl, kurs, datum, istKauf):
        self.anzahl = anzahl
        self.kurs = kurs
        self.datum = datum
...
class Position:
    ...
    kaufdatum = ""
    kaufdatumWert = 0
    def __init__(self, anzahl, kaufkurs, kaufdatum):
        self.anzahl = anzahl
        ...
        try:
            ...
            for i, transaktion in enumerate(transaktionen):
                if transaktion.datumWert > datumWert:
                    transaktionen.insert(i, Transaktion(int(self.anzahlEingabefeld.get()), float(self.preisEingabefeld.get().replace(',','.')), datum, True))
                    eingetragen = True
                    break
            if not eingetragen:
                transaktionen.append(Transaktion(int(self.anzahlEingabefeld.get()), float(self.preisEingabefeld.get().replace(',','.')), datum, True))
            update()
            root.attributes('-disabled', 0)
            self.fenster.destroy()
        except:
...
class VerkaufFenster:
    ...
    def bestaetigung(self):
        try:
            ...
            for i, transaktion in enumerate(transaktionen):
                if transaktion.datumWert > datumWert:
                    transaktionen.insert(i, Transaktion(int(self.anzahlEingabefeld.get()), float(self.preisEingabefeld.get().replace(',','.')), datum, False))
                    eingetragen = True
                    break
            if not eingetragen:
                transaktionen.append(Transaktion(int(self.anzahlEingabefeld.get()), float(self.preisEingabefeld.get().replace(',','.')), datum, False))
            root.attributes('-disabled', 0)
            self.fenster.destroy()
            update()
        except:
...
class VerkaufSimulationErgebnisFenster:
    ...
    def __init__(self, anzahl, preis):
        ...
        chargenListBox=tk.Listbox(chargenListboxFrame, width=100)  
        ...
        for position in aktuellePositionen:
            ...
            if position.anzahl <= verkaufAnzahl:
                ...
                verkaufAnzahl = 0
            gewinn += (preis-position.kaufkurs-summeVorabpauschalenProAnteil)*anzahlZuVerkaufen
            chargenListBox.insert("end","["+str(position.kaufdatum)+"] "+str(anzahlZuVerkaufen)+" Anteile @ "+str(position.kaufkurs)+"€ - Gewinn pro Anteil: "+str(round(preis-position.kaufkurs-summeVorabpauschalenProAnteil,2)) + "€ (berücksichtigte Vorabpauschale pro Anteil: " + str(round(summeVorabpauschalenProAnteil,2))+ "€)")

            if verkaufAnzahl == 0:
                break
        tk.Label(self.fenster, text = "Realisierter Gewinn nach Abzug bezahlter Vorabpauschalen vor Teilfreistellung: "+str(gewinn)+"€").pack()
...
class SteuerberichtErgebnisFenster:
    ...
    def __init__(self, jahr):
        ...
        chargenListBox=tk.Listbox(chargenListboxFrame, width=200)  
        chargenListBox.pack(side="left")
        ...
        # sets the geometry of toplevel
        self.fenster.geometry("1300x400")
        self.fenster.protocol("WM_DELETE_WINDOW", self.onClosingKaufFenster)
...
transaktionenListBox=tk.Listbox(transaktionenListboxFrame, width=40)  
...
vorabpauschalenListBox=tk.Listbox(vorabpauschalenListboxFrame, width=40)  
